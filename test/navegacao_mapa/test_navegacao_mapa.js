...
async function main() {
  ...
  const { sequelize, Artista, Utilizador, Categoria, Spot } = db;
  ...
  console.log("[+] Servidores ativos! A iniciar o teste de navegação visual...");
  let failed = false;
  ...
  try {
    ...
    tempCategory = await Categoria.create({
      nome_categoria: `Cat_UI_Map_${Date.now()}`
    });
    ...
    tempArtist = await Artista.create({
      numero_licenca: `LIC-UI-MAP-${Date.now()}`,
    ...
    tempArtistUser = await Utilizador.create({
      email: `artist_ui_map_${Date.now()}@example.com`,
      password: tempPasswordHashed,
      tipo: 'artista',
      data_registo: new Date(),
      nome_utilizador: `artist_ui_map_${Date.now()}`,
      id_artista: tempArtist.id_artista
    });

    // 4. Create temporary spot in unique Porto coordinates
    tempSpot = await Spot.create({
      localizacao: `Spot_UI_Map_${Date.now()}`,
      longitude: -8.6499,
      latitude: 41.1685,
      abertura: "09:00:00",
      fecho: "21:00:00"
    ...
    console.log("[*] A capturar ecrã com o mapa inicial...");
    const screenshotMapPath = path.join(screenshotsDir, "1_mapa_inicial.png");
    fs.writeFileSync(screenshotMapPath, await driver.takeScreenshot(), 'base64');
    console.log(`[+] Screenshot 1 (Mapa Inicial) guardado em: ${screenshotMapPath}`);

    console.log("\n[3] A procurar e selecionar o marcador do Spot correto no mapa...");
    const markers = await driver.wait(until.elementsLocated(By.css("img.leaflet-marker-icon")), 10000);
    ...
    for (const m of markers) {
      try {
        ...
        // Continue iterating if single click encounters transient issue
      }
    }

    if (!found) {
      throw new Error(`Não foi possível encontrar o marcador da localização "${tempSpot.localizacao}" no mapa.`);
    }
    ...
    console.log("[*] A capturar ecrã do spot selecionado com barra lateral...");
    const screenshotSpotPath = path.join(screenshotsDir, "2_spot_selecionado.png");
    fs.writeFileSync(screenshotSpotPath, await driver.takeScreenshot(), 'base64');
    console.log(`[+] Screenshot 2 (Spot Selecionado) guardado em: ${screenshotSpotPath}`);

    console.log("\n[+] TESTE DE NAVEGAÇÃO VISUAL NO MAPA CONCLUÍDO COM SUCESSO!");

  } catch (err) {
    console.error("\n[-] Ocorreu um erro durante o teste do mapa:", err);
    failed = true;
  } finally {
