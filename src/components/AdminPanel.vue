...
onMounted(async () => {
  ...
  // Fetch reservations, artists, spots, and categories
  try {
    const [resReservas, resArtists, resSpots, resCategories] = await Promise.all([
      api.get('/reservas'),
      api.get('/artistas'),
      api.get('/spots'),
      api.get('/categorias')
    ...
    reservationsList.value = resReservas.data || []
    artistsList.value = resArtists.data || []
    spotsList.value = resSpots.data.data || []
    categoriesList.value = resCategories.data || []
  } catch (error) {
      ...
      <CardBox class="overflow-hidden border border-gray-200 bg-white shadow-xl rounded-2xl dark:border-slate-700 dark:bg-slate-800">
        ...
        <div class="divide-y divide-gray-100 dark:divide-slate-700">
          <div v-for="artist in paginatedArtists" :key="artist.id_utilizador"
            ...
            <div class="col-span-3 text-gray-600 font-medium dark:text-slate-400 truncate">{{ artist.Artista?.numero_licenca || 'N/A' }}</div>
            <div class="col-span-3 text-gray-600 dark:text-slate-400 truncate">{{ artist.Artista ? getCategoryName(artist.Artista.categoria_id) : 'N/A' }}</div>
            <div class="col-span-2 text-gray-500 dark:text-slate-500">{{ artist.Artista ? formatDate(artist.Artista.validade_licenca) : 'N/A' }}</div>
